...
def HungarianCostBatch(num_classes, target_num = 100, pos_weight = 1., iou_weight = 1., class_weight = 1.):
  ...
  costs = tf.keras.layers.Lambda(lambda x, n: tf.map_fn(func, (x[0], x[1], x[2], x[3]), fn_output_signature = tf.RaggedTensorSpec(shape = (tf.shape(x[0])[1], None), dtype = tf.float32)), arguments = {'n': target_num})([bbox_pred, labels_pred, bbox_gt, labels_gt]);
